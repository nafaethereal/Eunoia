...
import { getAuth, signInWithPopup, signInWithRedirect, getRedirectResult, GoogleAuthProvider, onAuthStateChanged, signOut, setPersistence, browserLocalPersistence } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js";
...
onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
        ...
        if (isDashboard) {
            // Check if we just signed out or if there's truly no session
...
function loadGallery(uid, filter = 'all') {
    const grid = document.getElementById('gallery-grid');
    if (grid) grid.innerHTML = '<div class="loading-state">Loading gallery...</div>';

    if (!uid) {
        console.warn("loadGallery called without UID");
        return;
    }
    ...
    if (filter === 'all') {
        ...
        q = query(collectionRef, where("uid", "==", uid), where("category", "==", filter), orderBy("createdAt", "desc"));
    }
...
// Tambahkan pengecekan if (journalPhotoInput) di sini
if (journalPhotoInput) {
    journalPhotoInput.addEventListener('change', async (e) => {
        ...
        // ... sisa kode di dalam listener
    });
}


if (removeJournalPhotoBtn) {
...
const closeEditGalleryModalBtn = document.querySelector('.close-edit-gallery-modal');
if (closeEditGalleryModalBtn) {
    closeEditGalleryModalBtn.addEventListener('click', () => editGalleryModal.classList.add('hidden'));
}
